    ...
    fun onWorldRender(matrices: PoseStack, camera: Camera) {
        ...
        if (c.fuelPearlEnabled) {
            ...
            if (sol != null) {
                ...
                PearlWaypoints.drawWaypoint(matrices, immediate, sol.aimPoint, color, c.fuelPearlSize.toDouble(), c.pearlShape)
                if (c.fuelPearlTimer) {
                    ...
                    PearlWaypoints.drawLabel(matrices, immediate, sol.aimPoint, label, c.pearlTimerScale, c.pearlTimerPos)
                }
            }
        }
        ...
        if (c.fuelCellBeamsEnabled) {
            ...
            if (giants.isNotEmpty()) {
                ...
                for (g in giants) {
                    ...
                    KuudraCrateWaypoints.drawHorizontalCircle(matrices, immediate, pos, KuudraCrateWaypoints.DRAG_DIST, dc)
                }
                ...
                for (z in KuudraGiants.zombies) {
                    ...
                    KuudraCrateWaypoints.drawWireframeBox(matrices, immediate, z.boundingBox.inflate(0.1) as AABB, cc)
                }
            }
            val cells = fuelCells
