...
object HelixWaypoints {
    ...
    fun onWorldRender(matrices: PoseStack, camera: Camera) {
        ...
        boxEdges(immediate.getBuffer(FamilyRenderTypes.LINES), entry, x1, y1, z1, p.r, p.g, p.b, 1f)
        immediate.endBatch(FamilyRenderTypes.LINES)
        boxEdges(immediate.getBuffer(FamilyRenderTypes.LINES_NO_DEPTH), entry, x1, y1, z1, p.r, p.g, p.b, 1f)
        ...
        PearlWaypoints.drawLabel(matrices, immediate, label, "§e§l> §7${routePos + 1}/${route.size} $code${p.name} §7(${dist.toInt()}m)", scale, 2)
