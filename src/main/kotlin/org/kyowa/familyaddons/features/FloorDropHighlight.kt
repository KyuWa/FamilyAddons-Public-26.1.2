...
object FloorDropHighlight {
    ...
    fun onWorldRender(matrices: PoseStack, camera: Camera) {
        ...
        for (t in targets.values) {
            ...
            boxEdges(immediate.getBuffer(FamilyRenderTypes.LINES), entry, x1, y1, z1, c[0], c[1], c[2], c[3])
            immediate.endBatch(FamilyRenderTypes.LINES)
            boxEdges(immediate.getBuffer(FamilyRenderTypes.LINES_NO_DEPTH), entry, x1, y1, z1, c[0], c[1], c[2], c[3] * 0.6f)
