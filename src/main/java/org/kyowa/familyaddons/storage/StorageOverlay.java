...
public final class StorageOverlay {
    ...
    // moment instead of being worked out again on every frame, and keyed by page
    // so pointing along a row does not start from scratch each time.
    ...
    public static void onGuiClosed(Screen closed) {
        ...
        if (keepMenuOpen) {
            ...
            Scheduler.scheduleTask(1, () -> {
                ...
                if (current != null && Minecraft.getInstance().screen instanceof AbstractContainerScreen<?> acs) {
        ...
        if (keepMenuOpen) {
            if (opened == null) {
                ...
                Scheduler.scheduleTask(40, () -> {
                    if (keepMenuOpen && Minecraft.getInstance().screen == null) {
                        keepMenuOpen = false;
